...
// Обработка команды /start
bot.start((ctx) => {
  const startAppParam = ctx.startPayload; // Получение параметра startapp
  ...
  if (startAppParam) {
    ctx.telegram.sendMessage(ctx.chat.id, 'Открываю веб-приложение...', {
      reply_markup: {
        inline_keyboard: [
          [{ text: '🌐 Play', web_app: { url: 'https://blockchainnewshubs.com/' } }]
        ]
    ...
    ctx.reply('Используйте команду /play для открытия веб-приложения.')
      .then(() => {
        console.log('Сообщение отправлено успешно');
      })
      .catch((error) => {
